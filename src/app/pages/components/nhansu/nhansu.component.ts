...
import { INhanSu } from '../../api/nhansu';
import { NhanSuService } from '../../service/nhansu.service';
...
export class NhansuComponent {
    ...
    edit(nhansu: INhanSu) {
        this.nhansuService.getById(nhansu.id).subscribe(
            data => {
                ...
                this.nhansu = data;
                this.fileOnly = data.avartar;
                this.selectRole = this.role.find(option => option.name === data.roleId);
    ...
    save() {
        if (this.fileOnly) {
            this.nhansu.avartar = this.fileOnly.name;
        }
        this.nhansu.passWord = "techstore@123"
        this.nhansu.chucVuId = this.selectChucVu.id;
        this.nhansu.roleId = this.selectRole.id;
